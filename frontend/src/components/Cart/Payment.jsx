...
const Payment = () => {
    ...
    const [userMetwallet,setUserMetwallet]=useState()
    ...
    useEffect(()=>{
        setUserMetwallet(user.metawallet)
    },[])
    const { error } = useSelector((state) => state.newOrder);

    // console.log("cartitems",cartItems)
    // console.log("in payment user", user)
    ...
    const submitHandler = async (e) => {
        e.preventDefault();
     setPaid(true)
        enqueueSnackbar("Ordred", { variant: "success" });

        const orderData={
            metawallet:userMetwallet,
            ...cartItems[0]
...
{paid ? <span className={`${payDisable ? "bg-primary-grey cursor-not-allowed" : "bg-primary-orange cursor-pointer"} w-1/2 sm:w-1/4 my-2 py-3 font-medium text-white shadow hover:shadow-lg rounded-sm uppercase outline-none text-center`}>paid</span> : <input type="submit" value={`Pay ₹${totalPrice.toLocaleString()}`} disabled={payDisable ? true : false} className={`${payDisable ? "bg-primary-grey cursor-not-allowed" : "bg-primary-orange cursor-pointer"} w-1/2 sm:w-1/4 my-2 py-3 font-medium text-white shadow hover:shadow-lg rounded-sm uppercase outline-none`} />}
